...
for (s in seq_along(subjects))
{
  ...
  avgNeut <- (NTT+NBT)/2
  avgI <- (mean(sub1$HIT,na.rm=TRUE) + mean(sub1$DIT,na.rm=TRUE))/2
  avgC <- (mean(sub1$HCT,na.rm=TRUE) + mean(sub1$DCT,na.rm=TRUE))/2
  ...
  DT <- subset(tmp, select=c(Subject, Session,Trial.Type, bias.score))
  HI <- subset(tmp, select=c(Subject, Session,Trial.Type, bias.score))
  ...
  AttenO <- avgNeut - avgC
  AttenD <- avgI - avgNeut
  datavalues <- data.frame("SID" = tmp$Subject[1], "Session" = tmp$Session[1], "happy_vig" = mean(as.numeric(x$bias.score)), "happy_avo" = mean(as.numeric(y$bias.score)), "disgust_vig" = mean(as.numeric(z$bias.score)), "disgust_avo" = mean(as.numeric(w$bias.score)), "AttentionO" = AttenO, "AttentionD" = AttenD)
